...
public class Main {
  ...
  public static void dfs(int i) {
    checked[i] = true; // 방문 했다!
    System.out.print(i + " "); // 출력~~~~
    
    for(int j = 1; j <= n; j++) {
      if(check[i][j] == 1 && checked[j] == false) { // 전체 그래프에서 방문하지 않았으면??? 다시 재귀 호출;
        dfs(j);
      }
    }
  }
  ...
  public static void bfs() {
    ...
    while(!queue.isEmpty()) {
      ...
      for(int j = 1; j <= n; j++) { // 순서대로 확인
        if(check[temp][j] == 1 && checked[j] == false) { 
          queue.offer(j); //해당 j 큐에 추가
          checked[j] = true; // 해당 j 방문 했음!
